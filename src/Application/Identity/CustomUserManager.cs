...
{
    private readonly UserStore<User, Role, AppDbContext, Guid, IdentityUserClaim<Guid>, UserRole, IdentityUserLogin<Guid>, IdentityUserToken<Guid>, IdentityRoleClaim<Guid>> _userStore;
    ...
    {
        _userStore = (UserStore<User, Role, AppDbContext, Guid, IdentityUserClaim<Guid>, UserRole, IdentityUserLogin<Guid>, IdentityUserToken<Guid>, IdentityRoleClaim<Guid>>)store;

        //_services = services;
    }

    public async Task<IdentityResult> CreateAndAddToRoleTransactionAsync(User user, string password, string role)
    {
        ThrowIfDisposed();
    ...
    public async Task<IdentityResult> CreateAndAddToRoleTransactionAsync(User user, string role)
    {
        //CREATING USER
        ThrowIfDisposed();
    ...
    {
        if (Store is not IUserSecurityStampStore<User> cast)
        {
...
#pragma warning restore IDE0066 // Convert switch statement to expression
    ...
    {
        if (Store is not IUserLockoutStore<User> cast)
        {
